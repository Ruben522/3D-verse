...
const createModel = async (userId, data) => {
    const {
        title, main_color, description, file_url,
    ...
    try {
        ...
        const modelResult = await client.query(
            ...
            RETURNING *`,
            [userId, title, main_color, description, file_url, main_image_url, video_url, license]
        );
...
const getModels = async ({ page = 1, limit = 20 }) => {
    ...
    return { page, limit: safeLimit, total, totalPages: Math.ceil(total / safeLimit), data: modelsResult.rows };
};
...
const deleteModel = async (modelId, user) => {
    ...
    try {
        ...
        const absoluteFolder = path.join(process.cwd(), path.dirname(result.rows[0].file_url));
...
const updateModel = async (modelId, user, data) => {
    ...
    try {
        ...
        const values = [];
        let index = 1;
        const allowedFields = ["title", "description", "main_color", "license", "video_url", "main_image_url"];
        ...
        const query = `UPDATE models SET ${fields.join(", ")}, updated_at = CURRENT_TIMESTAMP 
...
export { createModel, getModelById, getModels, deleteModel, updateModel, addLike, removeLike };