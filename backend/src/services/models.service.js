...
const createModel = async (userId, data) => {
  ...
  if (!file_url || typeof file_url !== "string" || file_url.trim() === "") {
    throw new Error("La URL del archivo principal es obligatoria.");
  }
...
const getModelById = async (modelId) => {
  ...
  const formattedModel = formatModelAuthor(updatedModel)
...
const getModels = async ({ page = 1, limit = 20 }) => {
  ...
  const formattedModels = models.map(formatModelAuthor);
...
const deleteModel = async (modelId, user) => {
  ...
  return {
    message: "Modelo y archivos eliminados correctamente",
  };
};
...
const deleteMainImage = async (modelId, user) => {
  ...
  if (!model.main_image_url)
    throw new Error("El modelo ya no tiene imagen principal");
