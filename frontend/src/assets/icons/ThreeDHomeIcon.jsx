...
const RocketModel = () => {
    ...
    useFrame((state, delta) => {
        // 1. Animación del fuego (parpadeo y cambio de tamaño simulado)
        if (flameRef.current) {
        ...
        // 2. Rotación continua y muy suave de todo el cohete
        if (groupRef.current) {
            groupRef.current.rotation.y += delta * 0.4;
        }
    });

    // Color primario (ajustado a tu paleta)
    ...
    return (
        <group ref={groupRef} rotation={[0.2, 0, 0.1]}>
            <Float speed={3} rotationIntensity={0.2} floatIntensity={1.5}>
                ...
                {/* Aletas (Generadas matemáticamente en 3 ángulos) */}
