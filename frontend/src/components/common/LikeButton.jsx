...
const LikeButton = ({ modelId, likesCount = 0 }) => {
    const { likedModels, toggleLike } = useLikes();
    const isLiked = likedModels.has(String(modelId));
        ...
        >
            {isLiked ? (
                ...
                <>
                    ...
                    <span className="text-gray-500 group-hover/like:text-red-500">{likesCount}</span>
                </>
            )}
        </button>
    );
