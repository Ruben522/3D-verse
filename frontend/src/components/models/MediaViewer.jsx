            ...
            {!isInteractive && (
              <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/5 hover:bg-black/10 transition-colors cursor-pointer" onClick={() => updateDetailUI("isInteractive", true)}>
                <div className="bg-white/90 backdrop-blur px-6 py-4 rounded-2xl shadow-xl flex flex-col items-center gap-2 transform transition-transform hover:scale-105">
                  ...
                  <span className="font-extrabold text-gray-800">{t("media_viwer.clic_interact")}</span>
                </div>
              </div>
            )}
          </div>
      ...
      {activeMediaTab === "modelos" && isInteractive && (
        <div className="mt-6 flex flex-col gap-6 pt-6 border-t border-gray-100">
          <div className="w-full">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4">{t("media_viwer.color_piece")}</p>
            <ColorSelector selectedColor={currentColor} onSelect={(c) => updateDetailUI("currentColor", c)} />
          </div>

          {detectedParts.length > 1 && (
            <div className="w-full">
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4">{t("media_viwer.parts")}</p>
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto pr-2 custom-scrollbar">
                <button onClick={() => updateDetailUI("selectedPart", null)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${selectedPart === null ? "bg-primary-600 text-white shadow-md border-primary-600" : "bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100"}`}>{t("media_viwer.all")}</button>
                {detectedParts.map((p) => (
